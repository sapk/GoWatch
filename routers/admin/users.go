...
func Users(ctx *macaron.Context, auth *auth.Auth, sess session.Store) {
	if err := auth.VerificationAuth(ctx, sess, []string{"admin.users"}); err != nil {
		return
	}
	fillGlobalPage(ctx, "admin_users")
	ctx.Data["users_count"], ctx.Data["Users"] = user.GetAll()
...
func UserAdd(ctx *macaron.Context, auth *auth.Auth, sess session.Store) {
...
func UserAddPost(ctx *macaron.Context, auth *auth.Auth, sess session.Store) {
	if err := auth.VerificationAuth(ctx, sess, []string{"add.user"}); err != nil {
		return
	}
	err := user.CreateUser(ctx.Query("username"), ctx.Query("password"), ctx.Query("email"), ctx.Query("role"), auth.GetRoles())
	if err != nil {
